...
def valid_ISIN?(str)
  ...
  return true unless str.length == 12

  iso_code = str.chars.take(2).join()
  return false unless valid_ISO?(iso_code)

  digitized = str.chars.map { |char| char.to_i(36) }.join
  passes_luhn?(digitized)
end
